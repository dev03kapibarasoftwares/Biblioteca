...
namespace Teste.Web.Host.Startup
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddAbpWithoutCreatingServiceProvider<TesteWebHostModule>(
...
app.Use(async (context, next) =>                {                    await next();                    if (context.Response.StatusCode == 404                        && !Path.HasExtension(context.Request.Path.Value)                        && !context.Request.Path.Value.StartsWith("/api/services", StringComparison.InvariantCultureIgnoreCase))                    {                        context.Request.Path = "/index.html";                        await next();                    }                });
            ...
            app.UseSwaggerUI(options =>
            {
                ...
                options.SwaggerEndpoint($"/swagger/{_apiVersion}/swagger.json", $"Teste API {_apiVersion}");
                options.IndexStream = () => Assembly.GetExecutingAssembly()
                    .GetManifestResourceStream("Teste.Web.Host.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            services.AddSwaggerGen(options =>
            {
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"Teste.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"Teste.Web.Core.xml";
